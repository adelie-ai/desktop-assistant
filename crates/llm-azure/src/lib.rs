...
pub struct AzureClient {
    ...
    /// hosted-search request shape, so this client does not implement
...
impl AzureClient {
    ...
    /// factory-shape parity only; this client does not implement
...
mod tests {
    ...
    fn azure_never_reports_hosted_tool_search() {
        // Azure Chat Completions has no hosted-search request shape, so this
        // client does not implement `HostedToolSearch`. Reporting the
        // capability would send the whole tool fleet in one flattened request
        // with no tool-search entry. The builder still accepts the flag for
        // factory-shape parity; the client must ignore it.
        let plain = AzureClient::new("k".into());
        assert!(plain.hosted_tool_search().is_none());
        ...
        assert!(
            !opted_in.hosted_tool_search().is_some(),
