...
struct LegacyPreference {
    ...
    #[allow(dead_code)]
    updated_at: u64,
    // Legacy precomputed embedding; no longer carried over — the background
    // backfill task regenerates vectors after migration.
    #[serde(default)]
    #[allow(dead_code)]
    ...
    if preferences_path.exists() {
        ...
        if !content.trim().is_empty() {
            ...
            for pref in prefs.items {
                ...
                // Embeddings are regenerated by the background backfill task;
                // any precomputed vector in the legacy JSON is discarded.
                if let Err(e) = kb_store.write(entry).await {
    ...
    if memory_path.exists() {
        ...
        if !content.trim().is_empty() {
            ...
            for mem in memories.items {
                ...
                // Embedding regenerated by the background backfill task.
                if let Err(e) = kb_store.write(entry).await {
