...
async fn a_fetch_records_an_open_only_for_an_entry_offered_in_the_same_turn() {
    ...
    with_user_id(UserId::new(ALICE), async {
        ...
        let again = log
            .record_opened(CONV.to_string(), vec!["kb-offered".to_string()], Situation::new())
            .await
...
async fn an_offer_in_one_conversation_is_not_taken_up_in_another() {
    ...
    with_user_id(UserId::new(ALICE), async {
        log.record_offered(OfferScope::recall("conv-a"), vec!["kb-1".to_string()])
            ...
            .expect("offer recorded");
        let elsewhere = log
            .record_opened("conv-b".to_string(), vec!["kb-1".to_string()], Situation::new())
            .await
        ...
        let here = log
            .record_opened("conv-a".to_string(), vec!["kb-1".to_string()], Situation::new())
            .await
            .expect("read succeeds");
        assert_eq!(here, 1);
    })
...
async fn a_new_recall_block_replaces_the_previous_turns_standing_offers() {
    ...
    with_user_id(UserId::new(ALICE), async {
        ...
        assert_eq!(
            log.record_opened(CONV.to_string(), vec!["kb-last-turn".to_string()], Situation::new())
                .await
                .expect("read succeeds"),
            0,
        ...
        assert_eq!(
            log.record_opened(CONV.to_string(), vec!["kb-this-turn".to_string()], Situation::new())
                .await
                .expect("read succeeds"),
            1
        );
...
async fn an_offer_standing_in_two_conversations_can_be_taken_up_in_either() {
    ...
    with_user_id(UserId::new(ALICE), async {
        ...
        assert_eq!(
            log.record_opened("conv-a".to_string(), vec!["kb-broad".to_string()], Situation::new())
                .await
                .expect("read succeeds"),
            1,
        ...
        assert_eq!(
            log.record_opened("conv-b".to_string(), vec!["kb-broad".to_string()], Situation::new())
                .await
                .expect("read succeeds"),
            1,
...
async fn a_conversation_cannot_accumulate_standing_offers_without_limit() {
    ...
    with_user_id(UserId::new(ALICE), async {
        assert_eq!(
            log.record_opened(CONV.to_string(), vec![ids[over - 1].clone()], Situation::new())
                .await
                .expect("read succeeds"),
            1,
...
async fn a_cross_tenant_read_of_a_use_record_returns_nothing() {
    ...
    with_user_id(UserId::new(ALICE), async {
        log.record_offered(OfferScope::recall(CONV), vec!["kb-alice".to_string()])
            .await
            .expect("offer recorded");
        log.record_opened(CONV.to_string(), vec!["kb-alice".to_string()], Situation::new())
            .await
            .expect("open recorded");
        log.record_mark(MarkRequest {
            entry_ids: vec!["kb-alice".to_string()],
