...
mod tests {
    ...
    #[tokio::test]
    async fn bedrock_call_records_the_provider_request_id() {
        use tracing::Instrument;

        // `Converse` (the non-streaming path): a full, proven round trip
        // (`non_streaming_dispatch_inside_the_budget_returns_the_answer`
        // above uses the same mock shape), so the capture is exercised
        // through a real success response rather than an error body.
        let server = httpmock::MockServer::start();
        let converse = server.mock(|when, then| {
            when.method(httpmock::Method::POST)
                .path_matches(r"/converse$");
            then.status(200)
                .header("content-type", "application/json")
                .header("x-amzn-requestid", "req-test-bedrock-abc")
                .body(CONVERSE_RESPONSE_BODY);
        });

        let client = non_streaming_client(&server.url(""), 5);
        ...
        assert_eq!(
            ...
            Some("req-test-bedrock-abc"),
            "the connector must record Bedrock's own request id onto the span"
        );
    }

    #[tokio::test]
    async fn bedrock_call_without_a_request_id_records_nothing() {
        use tracing::Instrument;

        // `RequestId::request_id()` answers `None` when the SDK never learns
        // one - not only on a header the wire never sent, but also when the
        // service could not be reached. A response with no
        // `x-amzn-requestid`/`x-amz-request-id` header exercises the same
        // `None` arm a real unreachable-service failure would take.
        let server = httpmock::MockServer::start();
        server.mock(|when, then| {
            when.method(httpmock::Method::POST)
                .path_matches(r"/converse$");
        ...
        assert!(
            ...
            "no request id learned must leave the field empty, not record an empty string: {fields:?}"
        );
    }

    #[tokio::test]
    async fn bedrock_call_neutralises_a_control_character_in_the_request_id() {
        use tracing::Instrument;
        ...
        server.mock(|when, then| {
            ...
            then.status(200)
                ...
                // is what proves the connector routes the id through
                // `Safe::name` rather than passing it through raw.
                .header("x-amzn-requestid", "req\ttest")
                .body(CONVERSE_RESPONSE_BODY);
        });
    ...
    #[tokio::test]
    async fn bedrock_streaming_call_records_the_provider_request_id() {
        use tracing::Instrument;

        // `ConverseStream` (the streaming path, `dispatch_streaming`). The
        // AWS SDK resolves `request.send()` from the response headers alone
        // - `de_converse_stream_http_response` wraps the body into a lazy
        // `EventReceiver` without reading it - so a 200 with no real
        // event-stream body is enough to prove the capture runs before any
        // event is decoded. A model absent from both deny-lists, with no
        // tools, takes this path rather than `dispatch_non_streaming`.
        let server = httpmock::MockServer::start();
        server.mock(|when, then| {
            when.method(httpmock::Method::POST)
                .path_matches(r"/converse-stream$");
            then.status(200)
                ...
                .header("x-amzn-requestid", "req-test-bedrock-stream");
        });

        let client = BedrockClient::new(format!("AKIAIOSFODNN7EXAMPLE:{TEST_SECRET_ACCESS_KEY}"))
            .with_base_url("us-east-1")
            .__with_runtime_endpoint_for_test(server.url(""))
            .with_model("us.anthropic.claude-sonnet-4-6");
        let (capture, _guard, span) = request_id_capture();

        // The outer bound guards the test itself: if the lazy-body
        // assumption above is ever wrong, this fails on a timeout instead of
        // hanging the suite.
        ...
        assert_eq!(
            fields.get("provider_request_id").map(String::as_str),
            Some("req-test-bedrock-stream"),
            "the connector must record Bedrock's own request id onto the span \
             before the event-stream body is consumed"
        );
    }
}
