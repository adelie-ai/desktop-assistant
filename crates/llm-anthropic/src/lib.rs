...
impl AnthropicClient {
    ...
    async fn send_and_stream(
        ...
        // Capture Anthropic's own request id onto the open `llm.call` span
        // (#1152), before `bytes_stream()` consumes `response`. This is the
        // value to quote when opening a support ticket with Anthropic, and
        // the closest thing to end-to-end tracing this boundary allows.
...
mod tests {
    ...
    fn from_env_missing_key() {
        ...
        // test owns, no other test in this binary touches `ANTHROPIC_API_KEY`.
    ...
    #[tokio::test]
    async fn anthropic_call_records_the_provider_request_id() {
        ...
        server.mock(|when, then| {
            ...
            then.status(200)
                .header("content-type", "text/event-stream")
                .header("request-id", "req_test_abc123")
                .body(STUB_SSE_BODY);
        });
        ...
        assert_eq!(
            ...
            Some("req_test_abc123"),
            "the connector must record Anthropic's request-id header onto the span"
        );
    }

    #[tokio::test]
    async fn anthropic_call_without_a_request_id_header_records_nothing() {
        use tracing::Instrument;
    ...
    #[tokio::test]
    async fn anthropic_call_neutralises_a_control_character_in_the_request_id() {
        use tracing::Instrument;
