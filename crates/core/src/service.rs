...
10) Use built-in preference tools (builtin_preferences_remember/search/retrieve/delete/delete_old) for durable user preferences. \
11) Use built-in memory tools (builtin_memory_remember/search/retrieve/update/delete/delete_old) for durable factual memory and corrections. \
...
mod tests {
    ...
    async fn llm_input_includes_runtime_instruction_message() {
        ...
        assert!(
            messages[0]
                .content
                .contains("builtin_preferences_remember/search/retrieve/delete/delete_old")
        );
        assert!(
            messages[0]
                .content
                .contains("builtin_memory_remember/search/retrieve/update/delete/delete_old")
        );
    }
