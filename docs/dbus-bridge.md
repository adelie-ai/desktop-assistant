...
Routing D-Bus through the bridge → UDS → dispatch loop → handler makes the D-Bus
transport inherit idempotency, ack correlation, session scope, cancellation, and
live multi-client sync from the shared dispatcher. This page is the operator's
view.
...
the live D-Bus surface. The daemon does not claim the name and serves no D-Bus
surface of its own, so its systemd unit is `Type=simple` (not `Type=dbus` with
`BusName=`), and the `org.desktopAssistant` D-Bus activation file points at the
bridge.
...
- **Bridge restart cancels in-flight D-Bus turns.** Restarting the bridge drops
  its single UDS connection, which cancels any D-Bus-initiated turn still
