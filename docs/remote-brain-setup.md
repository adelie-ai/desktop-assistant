...
Point the overlay's `images[].newTag` at whatever you pushed (see
`deploy/k8s/overlays/example/kustomization.yaml`); the Deployment itself lives in
`deploy/k8s/base/daemon.yaml` and is not edited per environment.
...
extension via an initdb hook (`deploy/k8s/base/postgres.yaml` does this).
...
the cluster (`deploy/k8s/base/ollama.yaml`) and pull a small model:

```sh
kubectl apply -k deploy/k8s/overlays/example   # includes ollama.yaml
...
(generated by the overlay's `configMapGenerator` from its own `daemon.toml`),
mounted read-only into the pod. Transport,
...
```sh
# The whole stack is one kustomization: namespace, Postgres, Ollama, the
# generated daemon.toml ConfigMap and the Deployment. Copy overlays/example,
# set your namespace, image tag and daemon.toml in it, then:
